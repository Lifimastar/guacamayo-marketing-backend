        ...
        if event_type == 'payment_intent.succeeded':
            ...
            try:
                ...
                if e.code == 'PGRST116':
                    ...
                    logger.error(f"Supabase PostgREST Error fetching booking {booking_id} for PI {stripe_payment_intent_id}: {e}")
                    raise HTTPException(status_code=500, detail=f"Database error fetching booking: {e.message}")
                
            except Exception as e:
                logger.error(f"Unexpected Supabase Error fetching booking {booking_id} for PI {stripe_payment_intent_id}: {e}", exc_info=True)
                raise HTTPException(status_code=500, detail=f"Unexpected database error fetching booking: {e}")
            ...
            try:
                ...
                if e.code == 'PGRST116':
                    ...
                    raise HTTPException(status_code=500, detail=f"Database error fetching/updating payment on failed event: {e.message}")
            
            except Exception as e: 
                logger.error(f"Unexpected Supabase Error fetching/updating payment record for booking {booking_id} on failed event: {e}", exc_info=True)
